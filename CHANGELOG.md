...
- 增加全屏静默模式
- 版本提升到 0.1.24，用于发布新的 Windows NSIS 安装包、Tauri updater 签名文件和 latest.json。
## 2026-06-15

### 0.1.23 更新包

- 修复 Codex 状态库路径检测
- 版本提升到 0.1.23，用于发布新的 Windows NSIS 安装包、Tauri updater 签名文件和 latest.json。
## 2026-06-12
...
- 修复发布脚本哈希计算兼容性
- 版本提升到 0.1.21，用于发布新的 Windows NSIS 安装包、Tauri updater 签名文件和 latest.json。
## 2026-06-11

### 0.1.20 更新包

- 忽略过期的 Codex 订阅余量
- 版本提升到 0.1.20，用于发布新的 Windows NSIS 安装包、Tauri updater 签名文件和 latest.json。
...
- 只展示 Codex 订阅级余量
- 新增标准化发布脚本
- 版本提升到 0.1.19，用于发布新的 Windows NSIS 安装包、Tauri updater 签名文件和 latest.json。
## 2026-06-10
...
- 优化 Codex 余量快照读取，提升剩余额度展示稳定性。
- 版本提升到 `0.1.17`，用于发布新的 Windows NSIS 安装包、Tauri updater 签名文件和 `latest.json`。

## 2026-06-09

### 0.1.16 更新包

- 补强 GitHub 同步远端文件错误诊断，让远端文件读取失败时暴露更完整的上下文。
- 补强 GitHub 读取请求重试诊断，便于定位网络波动和 API 响应异常。
- 版本提升到 `0.1.16`，用于发布新的 Windows NSIS 安装包、Tauri updater 签名文件和 `latest.json`。

### 0.1.15 更新包

- 修复应用更新版本显示问题，并保留 Codex 用量旧快照，减少用量状态展示丢失。
- 修复数据同步一致性问题，优化 GitHub 同步分片体积。
- 增加 GitHub 同步 v2/v3 双模式和聚合同步 v3 设计，实现新旧同步格式并行支持。
- 版本提升到 `0.1.15`，用于发布新的 Windows NSIS 安装包、Tauri updater 签名文件和 `latest.json`。

### 0.1.14 更新包

- 调整今日 Token 小窗手动刷新同步逻辑，保持刷新期间的加载状态，并补充刷新耗时日志。
- 新增并合并今日 Token 小窗用量增量提示，让手动刷新后的用量变化更直观。
- 优化 GitHub 同步远端分片检查，记录远端 blob SHA，减少重复分片处理。
- 版本提升到 `0.1.14`，用于发布新的 Windows NSIS 安装包、Tauri updater 签名文件和 `latest.json`。

## 2026-06-08

### 0.1.13 更新包

- 修复应用更新后 GitHub 同步锁定状态残留的问题，避免同步状态卡在运行中。
- 修复主界面关闭后重新打开可能出现白屏的问题，增强窗口恢复稳定性。
- 修复 Codex 剩余用量桶切换时的显示抖动，让剩余额度展示更稳定。
- 版本提升到 `0.1.13`，用于发布新的 Windows NSIS 安装包、Tauri updater 签名文件和 `latest.json`。
