...
test("settings page is organized into clear grouped sections", () => {
  ...
  assert.ok(settingsPage.includes("type SettingsTabId"));
  assert.ok(settingsPage.includes("activeSettingsTab"));
  assert.ok(settingsPage.includes("settings-tab-list"));
  assert.ok(settingsPage.includes("settings-tab-button"));
  assert.ok(settingsPage.includes("settings-tab-panel"));
  assert.ok(settingsPage.includes("settings-overview-panel"));
  assert.ok(settingsPage.includes("settings-summary-grid"));
  assert.ok(settingsPage.includes("settings-summary-card"));
  assert.ok(settingsPage.includes("settings-overview-grid"));
  assert.ok(settingsPage.includes("settings-quick-grid"));
  assert.ok(settingsPage.includes('id: "overview"'));
  assert.ok(settingsPage.includes('id: "sources"'));
  assert.ok(settingsPage.includes('id: "devices"'));
  assert.ok(settingsPage.includes('id: "app"'));
  assert.ok(settingsPage.includes('id: "advanced"'));
  assert.ok(settingsPage.includes('setActiveSettingsTab("sources")'));
  assert.ok(settingsPage.includes('setActiveSettingsTab("devices")'));
  assert.ok(settingsPage.includes('setActiveSettingsTab("app")'));
  assert.ok(settingsPage.includes('setActiveSettingsTab("advanced")'));
  ...
  assert.ok(settingsPage.includes("应用"));
  assert.ok(settingsPage.includes("高级"));
  assert.ok(settingsPage.includes("本机 Agent 同步"));
  assert.ok(settingsPage.includes("管理多设备"));
  assert.ok(settingsPage.includes("打开应用设置"));
  assert.ok(styles.includes(".settings-section"));
  assert.ok(styles.includes(".settings-section-heading"));
  assert.ok(styles.includes(".settings-tab-list"));
  assert.ok(styles.includes(".settings-tab-button"));
  assert.ok(styles.includes(".settings-tab-panel"));
  assert.ok(styles.includes(".settings-overview-panel"));
  assert.ok(styles.includes(".settings-summary-grid"));
  assert.ok(styles.includes(".settings-summary-card"));
  assert.ok(styles.includes(".settings-overview-grid"));
  assert.ok(styles.includes(".settings-quick-grid"));
...
test("desktop tray exposes a today token pulse with historical average comparison", () => {
  ...
  assert.ok(trayStatus.includes("TOKEN_PULSE_DETAIL_CODEX_WIDTH"));
  ...
  assert.ok(trayStatus.includes("let anchor_size = anchor_window"));
  assert.ok(trayStatus.includes("unwrap_or((TOKEN_PULSE_COLLAPSED_WIDTH"));
  assert.ok(trayStatus.includes("requested_detail_width"));
  assert.ok(trayStatus.includes("requested_detail_height"));
  assert.ok(trayStatus.includes("let measured_detail_size = detail_window"));
  assert.ok(trayStatus.includes(".outer_size()"));
  assert.ok(trayStatus.includes(".ok()"));
  assert.ok(trayStatus.includes("unwrap_or((TOKEN_PULSE_DETAIL_CODEX_WIDTH"));
  assert.ok(trayStatus.includes("anchor_x + anchor_width - detail_width"));
  assert.ok(trayStatus.includes("pub fn open_token_pulse_home"));
  assert.ok(trayStatus.includes("pub fn hide_token_pulse_window"));
  ...
  assert.ok(tokenPulseWindow.includes("detailHeight"));
  assert.ok(tokenPulseWindow.includes("TOKEN_PULSE_DETAIL_CODEX_WIDTH"));
  assert.equal(tokenPulseWindow.includes("dragRegionAttributes"), false);
  ...
  assert.ok(tokenPulseWindow.includes("getCodexUsageLimits"));
  assert.ok(tokenPulseWindow.includes("runBackgroundSyncOnce"));
  assert.ok(tokenPulseWindow.includes("isManualRefreshing"));
  assert.ok(tokenPulseWindow.includes("setIsManualRefreshing(true)"));
  assert.ok(tokenPulseWindow.includes("setIsManualRefreshing(false)"));
  ...
  assert.ok(tokenPulseWindow.includes("logTokenPulsePerf(\"manual_refresh.background_sync\""));
  ...
  const tokenPulseManualSyncIndex = tokenPulseWindow.indexOf("await runBackgroundSyncOnce()");
  ...
  assert.ok(tokenPulseManualPulseRefreshIndex > tokenPulseManualSyncIndex);
  assert.ok(tokenPulseManualCodexAwaitIndex > tokenPulseManualSyncIndex);
  assert.ok(tokenPulseWindow.includes("showCodexUsageLimits"));
  assert.ok(tokenPulseWindow.includes("token-pulse-mini has-codex"));
  assert.ok(tokenPulseWindow.includes("token-pulse-detail has-codex"));
  assert.ok(tokenPulseWindow.includes("token-pulse-ring"));
  assert.ok(tokenPulseWindow.includes("TokenPulseRing"));
  ...
  assert.ok(tokenPulseWindow.includes("CodexUsageLimitsPanel"));
  assert.ok(tokenPulseWindow.includes("TOKEN_PULSE_MINI_CODEX_WIDTH = 444"));
  ...
  assert.ok(tokenPulseWindow.includes("setTodayDeltaTokens(deltaTokens)"));
  assert.ok(tokenPulseWindow.includes("window.setTimeout"));
  ...
  assert.ok(tokenPulseWindow.includes("token-pulse-action-panel"));
  ...
  assert.ok(styles.includes(".token-pulse-action-panel"));
  assert.ok(styles.includes(".token-pulse-action-button"));
  assert.ok(styles.includes(".token-pulse-action-icon"));
  assert.ok(styles.includes(".token-pulse-ring"));
  assert.ok(styles.includes(".token-pulse-ring-track"));
  assert.ok(styles.includes(".token-pulse-ring-progress"));
  assert.ok(styles.includes(".token-pulse-ring > span"));
  assert.ok(styles.includes("--ring-label-font-size: 0.68rem;"));
  assert.ok(styles.includes("--ring-label-compact-font-size: 0.58rem;"));
  assert.ok(styles.includes(".token-pulse-ring-value.is-compact"));
  assert.ok(styles.includes("--ring-label-font-size: 0.59rem;"));
  assert.ok(styles.includes("--ring-label-compact-font-size: 0.5rem;"));
  assert.ok(styles.includes("--ring-boost-orange: #f59e0b;"));
  assert.ok(styles.includes("--ring-boost-rose: #f43f5e;"));
  assert.ok(styles.includes("--ring-boost-gold: #facc15;"));
  assert.ok(styles.includes("--ring-boost-cyan: #22d3ee;"));
  assert.ok(styles.includes(".token-pulse-delta-chip"));
  assert.ok(styles.includes("position: absolute;"));
  assert.ok(styles.includes("right: 2px;"));
  ...
  assert.equal(styles.includes(".token-pulse-ring::after"), false);
  assert.ok(styles.includes(".token-pulse-codex"));
  assert.ok(styles.includes(".token-pulse-detail.has-codex"));
  assert.ok(styles.includes(".token-pulse-mini:not(.has-codex)"));
  assert.equal(styles.includes(".token-pulse-mini-subline"), false);
  assert.ok(styles.includes(".token-pulse-hour-chart"));
  assert.equal(styles.includes(".token-pulse-chart-average"), false);
  assert.equal(styles.includes(".token-pulse-legend i.average"), false);
  assert.equal(tokenPulseWindow.includes("日均水平"), false);
  assert.equal(tokenPulseWindow.includes("averageHourly"), false);
  assert.equal(tokenPulseWindow.includes("averageY"), false);
  assert.ok(styles.includes(".token-pulse-comparison"));
  ...
  assert.ok(dashboardService.includes("detailWidth"));
  assert.ok(dashboardService.includes("detailHeight"));
  assert.ok(dashboardService.includes("setTokenPulseDragging"));
  assert.ok(dashboardService.includes("set_token_pulse_dragging"));
  assert.ok(dashboardService.includes("openTokenPulseHome"));
  assert.ok(dashboardService.includes("open_token_pulse_home"));
  assert.ok(dashboardService.includes("hideTokenPulseWindow"));
  assert.ok(dashboardService.includes("hide_token_pulse_window"));
...
test("token pulse codex usage display is optional and defaults off", () => {
  ...
  assert.ok(displayPreference.includes("TokenScopeShowCodexUsageLimits"));
  assert.ok(displayPreference.includes('return value === "true"'));
  assert.ok(settingsPage.includes("showCodexUsageLimits"));
  assert.ok(settingsPage.includes("setShowCodexUsageLimits"));
  assert.ok(settingsPage.includes("显示 Codex 剩余用量"));
  assert.ok(settingsPage.includes("默认关闭"));
  assert.ok(tokenPulseWindow.includes("showCodexUsageLimits ?"));
...
test("settings page exposes github encrypted sync controls", () => {
  ...
  assert.ok(githubSyncPanel.includes("同步模式"));
  assert.ok(githubSyncPanel.includes("aggregate_v3"));
  assert.ok(githubSyncPanel.includes("detail_v2"));
  ...
  assert.ok(githubSyncPanel.includes("remote-device-actions"));
  ...
  assert.ok(githubSyncPanel.includes("getGitHubSyncRuntimeStatus"));
  assert.ok(githubSyncPanel.includes("syncRuntimeStatus"));
  assert.ok(githubSyncPanel.includes("syncElapsedSeconds"));
  assert.ok(githubSyncPanel.includes("actionsDisabled"));
  assert.ok(githubSyncPanel.includes("isAppUpdateBusy"));
  assert.ok(githubSyncPanel.includes("github-sync-live-status"));
  ...
  assert.ok(dashboardTypes.includes("GitHubSyncDataMode"));
  assert.ok(dashboardTypes.includes("data_mode: GitHubSyncDataMode"));
  assert.ok(dashboardTypes.includes("interface GitHubSyncRunResult"));
  assert.ok(dashboardTypes.includes("interface GitHubSyncRuntimeStatus"));
  ...
  assert.ok(styles.includes(".github-sync-live-status"));
  assert.ok(styles.includes(".sync-progress-bar"));
  assert.ok(styles.includes(".github-remote-device-list"));
  assert.ok(styles.includes(".remote-device-actions"));
});
...
test("application updater keeps explicit persisted state and complete progress", () => {
  ...
  assert.ok(settingsPage.includes("appUpdateVersionRange"));
  ...
  assert.ok(appShell.includes("appUpdateVersionRange"));
